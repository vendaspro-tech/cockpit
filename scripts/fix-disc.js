...
console.log('🔧 CORRIGINDO ESTRUTURA DO TESTE DISC\n')
...
async function fixDISC() {
  console.log('Criando estrutura corrigida do DISC...\n')
  ...
  DISC_QUESTIONS.forEach((item, idx) => {
    ...
    questions.push({
      id: `q${qNumber}_d`,
      text: `${qNumber}. ${item.text}\n[D] ${item.D}`,
      type: 'scale',
      order: qIndex++,
      required: true,
      scale_descriptors: SCALE_1_TO_4,
      metadata: {
        disc_question: qNumber,
        disc_profile: 'D',
        disc_statement: item.D
      }
    })
  ...
  const structure = {
    metadata: {
      ...
      description: 'Teste de perfil comportamental DISC adaptado para área comercial. Para cada questão, avalie TODAS as 4 afirmações de 1 a 4.',
      instructions: 'Este teste possui 24 questões situacionais. Para cada questão, você verá 4 afirmações (D, I, S, C). Atribua notas de 1 a 4 para cada afirmação: 4 = Mais se parece com você, 3 = Parece com você, 2 = Pouco se parece, 1 = Menos se parece. Responda pensando em como você realmente age, não como gostaria de agir.',
  ...
  console.log(`✅ Estrutura criada: ${questions.length} questões (24 situações x 4 perfis)`)
  ...
  console.log('\n⚠️  IMPORTANTE: Esta é uma solução temporária (workaround).')
  console.log('    A solução ideal seria implementar tipo "matrix_rating" no metamodelo.')
}
